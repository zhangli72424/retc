			...
			<scroll-view id="scrollview" :style="{height:style.contentViewHeight+'px'}" @scrolltolower="getList" scroll-y="true" scroll-with-animation="true">
				<block v-for="(item,index) in list" :key="index">
					<view class="item" hover-class="list-hover" @tap="handleclick(item)">
						<view class="left">
							<view class="name">{{title}}{{i18n.charge}}</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			handleclick(e) {
				let lets = e;
				lets.title = this.title;
				this.setDetails(lets)
				pageto(`log`)
			},
			getList(){
					...
					if(res.data.code){
						...
						if(this.page>1){
							...
							list.forEach((item,index)=>{
								list[index].number = parseFloat(item.number).toFixed(4)
							})
							...
							lists.forEach((item,index)=>{
								lists[index].number = parseFloat(item.number).toFixed(4)
							})
							this.list = lists
						}
					}else{
